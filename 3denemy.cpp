...
HRESULT C3denemy::Init()
{
    ...
    {
        C3denemy::MotionSet[nCount].Loop = false;
        C3denemy::MotionSet[nCount].NumKey = 0;
        for (int nKeySetCount = 0; nKeySetCount < MAX_KEYSET; nKeySetCount++)
        {
            MotionSet[nCount].KeySet[nKeySetCount].Frame = 0;

            for (int nKeyCount = 0; nKeyCount < ENEMY_MODEL; nKeyCount++)
            {
...
void C3denemy::Update()
{
    if (CScene::GetUpdateStat() == true)
    {
        ...
        if (Motion == C3denemy::MOTION_ACTION)
        {
...
C3denemy* C3denemy::Create(D3DXVECTOR3 pos,int MoveType)
{
    ...
    if (SUCCEEDED(D3Dplayer->Init()))
    {
        ...
        if (MoveType == 1)
        {
            D3Dplayer->SetType(TYPE::ENEMY_FLY);
        }

        if (MoveType == 2)
        {
            D3Dplayer->SetType(TYPE::ENEMY_NORMALMOVE);
...
void C3denemy::LoadPlayerData(void)
{
    ...
    m_pFile = fopen("data\\MODEL\\MODEL_ENEMY\\Charamotion.txt", "r");//ファイルを開く
        ...
        if (!strcmp(Datacheck, "SCRIPT"))
        {
            ...
            while (1)
            {
                ...
                if (!strcmp(Datacheck, "MODEL_FILENAME"))
                {
                    ...
                    fscanf(m_pFile, "%s", &C3denemy::m_aLoadEnemy[EnemyModelSave].filename[0]);
                    EnemyModelSave++;
                }
                    ...
                    while (1)
                    {
                        ...
                        if (!strcmp(Datacheck, "PARTSSET"))
                        {
                            ...
                            while (1)
                            {
                                ...
                                {
                                    fscanf(m_pFile, "%s", Datacheck);
                                    fscanf(m_pFile, "%d", &C3denemy::m_aLoadEnemy[nCnt].index);
                                }		// 種類
                                else if (!strcmp(Datacheck, "PARENT"))
                                {
                                    fscanf(m_pFile, "%s", Datacheck);
                                    fscanf(m_pFile, "%d", &C3denemy::m_aLoadEnemy[nCnt].parent);
                                }		// 幅
                                else if (!strcmp(Datacheck, "POS"))
                                {
                                    fscanf(m_pFile, "%s", Datacheck);
                                    fscanf(m_pFile, "%f", &C3denemy::m_aLoadEnemy[nCnt].pos.x);
                                    fscanf(m_pFile, "%f", &C3denemy::m_aLoadEnemy[nCnt].pos.y);
                                    fscanf(m_pFile, "%f", &C3denemy::m_aLoadEnemy[nCnt].pos.z);
                                }	// 高さ
                                else if (!strcmp(Datacheck, "ROT"))
                                {
                                    fscanf(m_pFile, "%s", Datacheck);
                                    fscanf(m_pFile, "%f", &C3denemy::m_aLoadEnemy[nCnt].rot.x);
                                    fscanf(m_pFile, "%f", &C3denemy::m_aLoadEnemy[nCnt].rot.y);
                                    fscanf(m_pFile, "%f", &C3denemy::m_aLoadEnemy[nCnt].rot.z);
                                }		// 奥行き
                ...
                if (!strcmp(Datacheck, "MOTIONSET"))
                {
                    ...
                    while (1)
                    {
                        ...
                        else if (!strcmp(Datacheck, "LOOP")) //ループの読み込み
                        {
                            ...
                            if (LoopType == 0)
                            {
                                C3denemy::MotionSet[nMotionCount].Loop = false;
                            }
                            else
                            {
                                C3denemy::MotionSet[nMotionCount].Loop = true;
                            }
                        }		// 種類
                        else if (!strcmp(Datacheck, "NUM_KEY")) //キー数の読み込み
                        {
                            fscanf(m_pFile, "%s", Datacheck);
                            fscanf(m_pFile, "%d", &C3denemy::MotionSet[nMotionCount].NumKey);
                        }		// 幅
                        ...
                        if (!strcmp(Datacheck, "KEYSET")) //キーセットの読み込み
                        {
                            ...
                            while (1)
                            {
                                ...
                                {
                                    fscanf(m_pFile, "%s", Datacheck);
                                    fscanf(m_pFile, "%d", &C3denemy::MotionSet[nMotionCount].KeySet[nKeySetCount].Frame);
                                }
                                ...
                                if (!strcmp(Datacheck, "KEY")) //キーの読み込み
                                {
                                    ...
                                    while (1)
                                    {
                                        ...
                                        {
                                            fscanf(m_pFile, "%s", Datacheck);
                                            fscanf(m_pFile, "%f", &C3denemy::MotionSet[nMotionCount].KeySet[nKeySetCount].aKey[nKeyCount].pos.x);
                                            fscanf(m_pFile, "%f", &C3denemy::MotionSet[nMotionCount].KeySet[nKeySetCount].aKey[nKeyCount].pos.y);
                                            fscanf(m_pFile, "%f", &C3denemy::MotionSet[nMotionCount].KeySet[nKeySetCount].aKey[nKeyCount].pos.z);
                                        }
                                        else if (!strcmp(Datacheck, "ROT"))
                                        {
                                            fscanf(m_pFile, "%s", Datacheck);
                                            fscanf(m_pFile, "%f", &C3denemy::MotionSet[nMotionCount].KeySet[nKeySetCount].aKey[nKeyCount].rot.x);
                                            fscanf(m_pFile, "%f", &C3denemy::MotionSet[nMotionCount].KeySet[nKeySetCount].aKey[nKeyCount].rot.y);
                                            fscanf(m_pFile, "%f", &C3denemy::MotionSet[nMotionCount].KeySet[nKeySetCount].aKey[nKeyCount].rot.z);
                                        }
    ...
    {
        //Xファイルの読み込み
        D3DXLoadMeshFromX(&C3denemy::m_aLoadEnemy[nInfoCount].filename[0], D3DXMESH_SYSTEMMEM, pDevice, NULL, &m_pBuffMat[nInfoCount], NULL, &m_nNumMat[nInfoCount], &m_pMesh[nInfoCount]);

        //位置の初期化
        m_aModel[nInfoCount].pos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
        m_aModel[nInfoCount].nIdxModelParent = C3denemy::m_aLoadEnemy[nInfoCount].parent;
        m_aModel[nInfoCount].pos = D3DXVECTOR3(C3denemy::m_aLoadEnemy[nInfoCount].pos.x, C3denemy::m_aLoadEnemy[nInfoCount].pos.y, C3denemy::m_aLoadEnemy[nInfoCount].pos.z);
        m_aModel[nInfoCount].rot = D3DXVECTOR3(C3denemy::m_aLoadEnemy[nInfoCount].rot.x, C3denemy::m_aLoadEnemy[nInfoCount].rot.y, C3denemy::m_aLoadEnemy[nInfoCount].rot.z);


        m_aModel[nInfoCount].pos += C3denemy::MotionSet[0].KeySet[0].aKey[nInfoCount].pos;
        m_aModel[nInfoCount].rot += C3denemy::MotionSet[0].KeySet[0].aKey[nInfoCount].rot;
    }
}
...
// モデルのモーション処理
//======================
void C3denemy::ModelMotion()
{
    // モデルのパーツ分繰り返す
    for (int nModelCount = 0; nModelCount < ENEMY_MODEL; nModelCount++)
    {
        // 現在のモーションのキーセットを取得
        const auto& currentKey = C3denemy::MotionSet[Motion].KeySet[m_nMotionCnt];
        const auto& nextKey = C3denemy::MotionSet[Motion].KeySet[(m_nMotionCnt + 1) % C3denemy::MotionSet[Motion].NumKey];
    ...
    // キーフレーム終了時の処理
    if (m_nFrameCnt >= C3denemy::MotionSet[C3denemy::Motion].KeySet[m_nMotionCnt].Frame)
    {
        m_nFrameCnt = 0;           // カウントを初期化
        m_nMotionCnt++;            // モーションのカウントを増加

        // モーションの終了条件をチェック
        if (m_nMotionCnt >= C3denemy::MotionSet[C3denemy::Motion].NumKey)
        {
            m_nMotionCnt = 0; // カウントを0にする

            // ループしない場合、モーションを停止
            if (!C3denemy::MotionSet[C3denemy::Motion].Loop)
            {
                C3denemy::MotionUse = false;    // モーションの判定をoffにする
...
// モーションの指定
//======================
void C3denemy::SetEnemyMotion(ENEMYMOTION motion)
{
    if (C3denemy::Motion != motion && C3denemy::MotionUse == false) //現在のモーションと違ったら
    {
        C3denemy::Motion = motion; //モーションを設定
        C3denemy::m_nMotionCnt = 0; //モーションのカウントを初期化
        C3denemy::m_nFrameCnt = 0;  //フレームのカウントの初期化

        // モデルのパーツ分繰り返して位置と回転を初期化
        for (int nModelCount = 0; nModelCount < ENEMY_MODEL; nModelCount++)
        {
            m_aModel[nModelCount].pos = m_aLoadEnemy[nModelCount].pos + C3denemy::MotionSet[motion].KeySet[0].aKey[nModelCount].pos;
            m_aModel[nModelCount].rot = m_aLoadEnemy[nModelCount].rot + C3denemy::MotionSet[motion].KeySet[0].aKey[nModelCount].rot;
        }
    }
}
...
// 敵の当たり判定
//===========================
bool C3denemy::Collision3DEnemy(D3DXVECTOR3* pPos, D3DXVECTOR3* pPosOld, D3DXVECTOR3* pMove, float fWidth, float fBlockHeight)
{
    ...
    // 右側当たり判定
    if (pPos->x - fWidth <= Pos.x + fBlockWidth && pPosOld->x - fWidth >= Pos.x + fBlockWidth && pPos->z - fHeight < Pos.z + fBlockDepth && pPos->z > Pos.z - fBlockDepth && pPos->y < Pos.y + fBlockHeight - 5.0f && pPos->y > Pos.y - fBlockHeight - 60.0f)
    {
