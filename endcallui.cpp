...
void CEndCallUI::Uninit()
{
    ...
    for (int nCntBG = 0; nCntBG < NUM_ICON; nCntBG++)
    {
        if (m_pEndTexBuff[nCntBG] != nullptr)
        {
            m_pEndTexBuff[nCntBG]->Release();
            m_pEndTexBuff[nCntBG] = nullptr;
        }
    }
...
void CEndCallUI::Draw()
{
    ...
    for (nCntBG = 0; nCntBG < NUM_ICON; nCntBG++)
    {
        ...
        {
            //テクスチャの設定
            pDevice->SetTexture(0, m_pEndTexBuff[nCntBG]);
...
CEndCallUI* CEndCallUI::Create(D3DXVECTOR3 pos, D3DXVECTOR3 size)
{
    ...
    for (nCntBG = 0; nCntBG < NUM_ICON; nCntBG++)
    {
        ...
        case 0: //通常ゴール
            ...
            break;
        case 1: //失敗時
            D3DXCreateTextureFromFile(CManager::GetRenderer()->GetDevice(), "data\\TEXTURE\\failed_window.png", &pTexture[nCntBG]);
            break;
        case 2: //特殊ゴール時
            D3DXCreateTextureFromFile(CManager::GetRenderer()->GetDevice(), "data\\TEXTURE\\spgoal_window.png", &pTexture[nCntBG]);
            break;
